...
$$\frac{d^2\tilde\Phi_x}{dz^2} - \alpha_x^2\,\tilde\Phi_x = -\frac{1}{D_x}\,\delta(z-z_0)$$

$$\alpha_x = \sqrt{s_r^2 + \frac{\mu_{ax}^\text{tot}}{D_x}}, \quad
\delta_x = \frac{1}{\sqrt{\mu_{ax}^\text{tot}/D_x}} = \sqrt{\frac{D_x}{\mu_{ax}^\text{tot}}}$$
...
La solution de l'EDO d'excitation est identique au cas sans fluorescence
(voir :doc:`08_da_2d_kienle_sans_fluo`) :

$$\tilde\Phi_x(s_r,z) = \frac{1}{2\alpha_x D_x}
\left[e^{-\alpha_x|z-z_0|} - e^{-\alpha_x(z+z_0+2z_{bx})}\right]$$
...
soit une combinaison de deux exponentielles $e^{-\alpha_x|z-z_s|}$.

Pour chaque terme source de la forme $f_s(z) = C_s\,e^{-\alpha_x|z-z_s|}$,
la solution particulière de l'EDO d'émission est (pour $\alpha_x \neq \alpha_m$) :

$$\tilde\Phi_m^\text{part}(z) = \frac{-C_s/D_m}{\alpha_m^2-\alpha_x^2}\,e^{-\alpha_x|z-z_s|}$$
...
**Réflectance d'excitation** (identique au cas sans fluorescence) :

$$\tilde R_x(s_r) = -D_x\,\frac{d\tilde\Phi_x}{dz}\bigg|_{z=0}
= \frac{1}{2}\,\frac{e^{-\alpha_x z_0}+e^{-\alpha_x(z_0+2z_{bx})}}{1+2A_xD_x\alpha_x}$$
...
$$R_m(r) = \frac{1}{2\pi}\int_0^\infty \tilde R_m(s_r)\,J_0(s_r\,r)\,s_r\,ds_r$$
...
indépendamment de la géométrie, ce qui en fait un observable robuste pour la
quantification du fluorophore.

En régime temporel, la transformée de Laplace inverse de $\tilde R_m(s_r,s)$ donne la
TPSF de fluorescence pour chaque $s_r$, permettant la reconstruction tomographique
résolue en temps.
...
- **Multicouches** : le passage en fréquences spatiales se généralise naturellement
  à $N$ couches par raccordement des solutions à chaque interface, sans nécessiter
  de série infinie de sources images.
- **SFDI** : $\tilde R_m(s_r)$ est directement l'observable expérimental en SFDI
  fluorescente, sans transformée inverse intermédiaire.
- **Régularisation** : travailler dans l'espace de Fourier permet d'appliquer des
  filtres spectraux (régularisation de Tikhonov) directement sur $s_r$.
